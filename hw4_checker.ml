...
declaration")
  ...
  | DeRef(e) ->
    ...
    (match t1 with
      | RefType(t) -> return t
      | _ -> error "Failed to dereference" )
  | SetRef(e1,e2) ->
    type_of_expr e1 >>= fun t1 ->
    type_of_expr e2 >>= fun t2 ->
    (match t1 with
      | RefType(t) -> if t = t2 then return UnitType else error "setref: arguments are different type"
  ...
  | EmptyList(t) ->
    return @@ ListType(t)
  | Cons(h, t) ->
    type_of_expr h >>= fun f1 ->
    type_of_expr t >>= fun f2 ->
    (match f2 with 
      | ListType(l) -> if l = f1 then return @@ ListType(f1) else error "cons: type of head and tail do not match"
      | _ -> error "cons: second argmument is not a ListType")
  | Null(e) ->
     type_of_expr e >>= fun t1 ->
     (match t1 with
      | ListType(_) -> return BoolType
      | _ -> error "null: expession is not ListType")
  | Hd(e) ->
    type_of_expr e >>= fun t1 ->
    (match t1 with
      | ListType(l) -> return l
      | _ -> error "hd: expression is not ListType")
  | Tl(e) ->
    type_of_expr e >>= fun t1 ->
    (match t1 with
      | ListType(_) -> return t1
      | _ -> error "tl: expression is not ListType")
  ...
  | Node(de, le, re) ->
    ...
    if t2 = t3 then
      (match t2 with
        | TreeType(l) -> if l = t1 then return t2 else error "node: argument types are different"
        | _ -> error "node: expected a TreeType")
    else error "node: argument types are different"
  | NullT(t) ->
    type_of_expr t >>= fun t1 ->
    (match t1 with
      | TreeType(_) -> return BoolType
      | _ -> error "nullt: expected a TreeType")
  | GetData(t) ->
    type_of_expr t >>= fun t1 ->
    (match t1 with
      | TreeType(l) -> return l
      | _ -> error "getdata: expected a TreeType")
  | GetLST(t) ->
    type_of_expr t >>= fun t1 ->
    (match t1 with
      | TreeType(_) -> return t1
      | _ -> error "getlst: expected a TreeType")
  | GetRST(t) ->
    type_of_expr t >>= fun t1 ->
    (match t1 with
      | TreeType(_) -> return t1
      | _ -> error "getrst: expected a TreeType") 
