(*I pledge my honor that I have abided by the Stevens Honor System. -Siddhanth Patel*)
open Ast
open Ds
...
let rec lookup_class : string -> class_env -> class_decl ea_result  = fun c_name c_env ->
  match c_env with
  	| (cname, decl)::t -> if c_name = cname then return (decl) else lookup_class c_name t
  	| _ -> error "lookup_class: class c_name not found"

let rec new_env_helper = fun fs ->
	(match fs with
	| [] -> return @@ EmptyEnv
	| head::tail -> (new_env_helper tail) >>= fun rest_env -> return @@ (ExtendEnv (head, RefVal (Store.new_ref g_store (NumVal 0)), rest_env)))

let new_env : string list -> env ea_result  = fun fs ->
	(match fs with
	| [] -> empty_env()
	| _::_ -> new_env_helper @@ List.rev fs)
...
let lookup_method : string -> string -> class_env ->
  ...
  (match List.assoc_opt c_name c_env with
  	| None -> None
  	| Some (_,_,mtd) -> (List.assoc_opt m_name mtd))
